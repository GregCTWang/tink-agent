...
def _silence_block(level=20):
    rng = np.random.default_rng(2)
    return (rng.standard_normal(800) * level).astype(np.int16)
...
def test_onset_gate_ignores_tone_active():
    g = DictationOnsetGate(90, 70, 100, 400, 16000, 800, 400, 60000)
...
def test_toggle_app_starts_and_stops_with_send():
    ...
    for _ in range(10):
        eng.handle_block(_silence_block())
...
def test_dictation_cancel_hold_sends_escape_then_release():
    ...
    for _ in range(2):
        eng.handle_block(_speech_block())
    kb.events.clear()
    eng.handle_block(_tone_block(3100))  # slot 3
...
def test_max_session_ends_with_cleanup():
    ...
    eng, scheduled = _engine_with_dictation(kb, config=c, delays=[])
    for _ in range(2):
        eng.handle_block(_speech_block())
    # 100 ms cap -> 2 blocks at 50 ms
    eng.handle_block(_speech_block())
    assert ("release", "m") in kb.events
